...
class UsersMovieTest < ActiveSupport::TestCase
  ...
  test "new movie is created when given a non-existent UPC" do
    movie = Movie.find_by_title("Blankman")
    ...
    Movie.expects(:lookup_on_amazon).at_least_once.returns(Hash.new)

    um = UsersMovie.new(:movie_attributes => movie_attributes)
  ...
  test "amazon is used to create a movie we can't find one in the db" do
    ...
    movie = Movie.find_by_title("Blankman [Extended Edition]")
    assert_not_nil movie
